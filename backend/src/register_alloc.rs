...
use middleend::ir::{InstUUID, BBIndex, BasicBlock};
...
impl LinearAllocator {
    ...
    fn allocate(&mut self, prog: &middleend::ir::Function) {
        for block in prog.blocks.iter() {
            for inst in block.iter() {
                ...
                let InstUUID(_, BBIndex(bb_index), inst_index) = inst.id;
    ...
    fn create_release(&mut self, reg: middleend::ir::Register, blocks: &Vec<BasicBlock>) {
        let InstUUID(_, BBIndex(bb_start), inst_start) = reg;
        ...
        for bb_index in bb_start..blocks.len() {
            for inst_index in inst_start..blocks[bb_index].len() {
                if self.liveness[bb_index][inst_index].contains(&reg) {
                    place = InstUUID(false, BBIndex(bb_index), inst_index);
                }
            }
        }
        let InstUUID(_, BBIndex(bb_index), inst_index) = place;
